...
/**
 * Relationship handler to return the taxonomy terms of commerce products.
 *
...
class CommerceProductTermData extends RelationshipPluginBase {
  ...
  /**
   * Constructs a ProductTermData object.
   *
  ...
  public function query() {
    ...
    if (!array_filter($this->options['vids'])) {
      $taxonomy_index = $this->query->addTable('taxonomy_index_commerce_product', $this->relationship);
      ...
      $query->addJoin($def['type'], 'taxonomy_index_commerce_product', 'tn', 'tn.tid = td.tid');
      $query->condition('td.vid', array_filter($this->options['vids']), 'IN');
      $query->addTag('taxonomy_term_access');
      $query->fields('td');
      $query->fields('tn', array('product_id'));
