...
(function($, Drupal) {
    var MdSliderPanel = function() {
        ...
        this.init = function() {
            ...
            this.setTabToolHeight();
        };
        this.setTabToolHeight = function(){
            // reset tabs-head height
            $('.md-tab-tools ul.md-tabs-head').css('height', '');
            setTimeout(function(){
                var ul_height = $('.md-tab-tools .md-tabs-head').outerHeight(),
                    tool_height = 48,
                    rows = Math.floor(ul_height/tool_height),
                    button_add_down = (jQuery('.add-tab').outerHeight() > 60) ? true : false;
                $('.md-tab-tools, .md-tab-tools ul.md-tabs-head').height(rows  * tool_height);
                if(rows > 1 || button_add_down){
                     $('.md-tab-tools .tab-item').addClass('border-bottom');
                     $('.md-tab-tools').addClass('fix-multi-rows').height((rows + 1)  * tool_height);
                     if(rows > 1){
                        if(!$('#vertical-sort').length){
                            $('.add-tab').append('<a id="vertical-sort" href="#">Vertical sort mode</a>');
                        }
                        $('#vertical-sort').click(function(){
                            if($(this).hasClass('vertical-active')){
                               $('#vertical-sort').text('Vertical sort mode'); 
                               $('.md-tab-tools').removeClass('tab-vertical-list');
                               $(this).removeClass('vertical-active');
                               self.tabs.find(".ui-tabs-nav").sortable("option", "axis", "");
                            } else {                                
                                $('#vertical-sort').text('Stop vertical sort mode'); 
                                $('.md-tab-tools').addClass('tab-vertical-list');
                                $(this).addClass('vertical-active');
                                self.tabs.find(".ui-tabs-nav").sortable("option", "axis", "y");
                            }
                            return false;
                        });
                     } else {
                         $('#vertical-sort').remove();
                     }
                     self.tabs.find(".ui-tabs-nav").sortable("option", "axis", (rows == 1) ? "x" : "");
                }else{
                    $('.md-tab-tools .tab-item').removeClass('border-bottom');
                    $('.md-tab-tools, .md-tab-tools ul.md-tabs-head').height(tool_height);
                    $('.md-tab-tools').removeClass('fix-multi-rows');
                    self.tabs.find(".ui-tabs-nav").sortable("option", "axis", "x");
                    $('#vertical-sort').remove();
                }
            }, 50);            
        };
        this.initTab = function() {
            ...
            self.tabs.find(".ui-tabs-nav").sortable({
                axis: "x",
                placeholder: "ui-placeholder tab-item",
                stop: function() {
                    self.tabs.tabs("refresh");
                }
            });
        ...
        this.addTab = function() {
            ...
            //$(tabNav).insertBefore($('#add_tab').parent());
        ...
        this.setSlideSettingValue = function(setting) {
            ...
            $("#link_redirect").val(setting.link_redirect);
